...
const MyOrders = () => {
    ...
    return (
        <div className='px-4 md:px-16 mt-15 lg:px-24 xl:px-32 py-6'>
                        ...
                        <div key={id} className='bg-white rounded-lg border border-gray-200 p-5 flex flex-col max-w-4xl'>
                            ...
                            <div className='flex flex-col gap-2 items-end'>
                                ...
                                <div className='text-xs text-gray-500 mt-2 text-right'>
                                    {/* Handle Date formatting safely in case mock data date is invalid */}
                                    <div>Credential Purchased: {order.createdAt ? format(new Date(order.createdAt), 'MMM d, yyyy') : 'N/A'}</div>
                                </div>
                            </div>
                            ...
                            {isExpanded && (
                                <div className='mt-4 md:mt-0 pt-4'>
                                    <div className='space-y-2'>
                                        {credential.updatedCredential.map((cred, index) => (
                                            <div key={`${cred.name}-${index}`} className='flex items-center justify-between gap-3 bg-gray-50 rounded-md p-2'>
                                                <div>
